...
Write a function name `plus` that returns the sum of its arguments, where the arguments are being passed in curried style, such that for all numbers `x` and `y`, `plus()(x)(y) === x + y`.
